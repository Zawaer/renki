...
const Env = z.object({
  ...
  RENKI_REPOS_ROOT: z.string().min(1).default(resolve(homedir(), "coding")),
  /** Where the daemon keeps its SQLite db and per-session worktrees. */
  RENKI_DATA_DIR: z.string().min(1).default(resolve(process.cwd(), "data")),
  /**
   ...
   * step (`renki token` prints whichever one is active).
   */
  RENKI_AUTH_TOKEN: z.string().min(1).optional(),
  /** Idle minutes before the take-control lock auto-releases. */
  RENKI_CONTROL_IDLE_MINUTES: z.coerce.number().int().positive().default(15),
  /**
   ...
   */
  RENKI_LIVE_IDLE_MINUTES: z.coerce.number().min(0).default(60),
  /**
  ...
  RENKI_PORT: z.coerce.number().int().positive().default(4517),
  /** Address to bind. Default localhost; set to the tailnet IP in Step 4. */
  RENKI_HOST: z.string().min(1).default("127.0.0.1"),
  /**
   ...
   */
  RENKI_FORCE_PERMISSION_PROMPTS: z.string().optional(),
  /**
   ...
   */
  RENKI_ENABLE_RTK: z.string().optional(),
  ...
  RENKI_RTK_BIN: z.string().min(1).default("rtk"),
  /** Expo push endpoint (override to a mock in tests). */
  RENKI_EXPO_PUSH_URL: z.string().url().default("https://exp.host/--/api/v2/push/send"),

  // ── multi-account usage rotation (cswap) ──
  /** Enable automatic account rotation. Off by default. */
  RENKI_ACCOUNT_ROTATION: z.string().optional(),
  /** cswap executable (name on PATH or absolute path). */
  RENKI_CSWAP_BIN: z.string().min(1).default("cswap"),
  /** Switch when the active account's 5h or 7d usage reaches this percent. */
  RENKI_ROTATION_THRESHOLD: z.coerce.number().min(1).max(100).default(90),
  /** Minutes between switches (avoids flip-flopping near the threshold). */
  RENKI_ROTATION_COOLDOWN_MINUTES: z.coerce.number().positive().default(5),
  /** How often to poll cswap for usage. */
  RENKI_ROTATION_POLL_SECONDS: z.coerce.number().int().positive().default(60),
  /** cswap --switch strategy. */
  RENKI_ROTATION_STRATEGY: z.enum(["best", "next-available"]).default("best"),
  /** On a rate-limit failure, switch accounts and retry the prompt once. */
  RENKI_ROTATION_AUTORETRY: z.string().optional(),
  /**
   ...
   */
  RENKI_ROTATION_CONFIG: z.string().optional(),
  /**
   ...
   */
  RENKI_USAGE_CONFIG: z.string().optional(),
  /** Base URL for the usage endpoint (override for testing). */
  RENKI_USAGE_BASE_URL: z.string().url().default("https://claude.ai"),
  /** Browser channel the guided browser login reuses ("chrome"/"msedge"; empty = bundled Chromium). */
  RENKI_USAGE_LOGIN_CHANNEL: z.string().default("chrome"),
  /** How long the guided-login browser waits for you to finish signing in. */
  RENKI_USAGE_LOGIN_TIMEOUT_SECONDS: z.coerce.number().int().positive().default(180),
});
 ...
 * workspace root. This matters because `pnpm --filter @renki/daemon dev` runs with
...
/**
 * Resolve the daemon's auth token: an explicit RENKI_AUTH_TOKEN always wins.
...
function resolveAuthToken(envToken: string | undefined, dataDir: string): string {
  ...
  logger.info("no RENKI_AUTH_TOKEN set — generated one", { path: tokenPath });
  return generated;
}
...
export function loadConfig(): Config {
  ...
  const dataDir = resolve(env.RENKI_DATA_DIR);
  return {
    reposRoot: resolve(env.RENKI_REPOS_ROOT),
    dataDir,
    dbPath: resolve(dataDir, "renki.sqlite"),
    worktreesDir: resolve(dataDir, "worktrees"),
    authToken: resolveAuthToken(env.RENKI_AUTH_TOKEN, dataDir),
    controlIdleMs: env.RENKI_CONTROL_IDLE_MINUTES * 60_000,
    liveIdleMs: env.RENKI_LIVE_IDLE_MINUTES * 60_000,
    trashRetentionMs: env.RENKI_TRASH_RETENTION_DAYS * 24 * 60 * 60_000,
    shutdownGraceMs: env.RENKI_SHUTDOWN_GRACE_SECONDS * 1000,
    permissionTimeoutMs: env.RENKI_PERMISSION_TIMEOUT_SECONDS * 1000,
    port: env.RENKI_PORT,
    host: env.RENKI_HOST,
    forcePermissionPrompts: env.RENKI_FORCE_PERMISSION_PROMPTS === "1" || env.RENKI_FORCE_PERMISSION_PROMPTS === "true",
    enableRtk: env.RENKI_ENABLE_RTK === "1" || env.RENKI_ENABLE_RTK === "true",
    rtkBin: env.RENKI_RTK_BIN,
    expoPushUrl: env.RENKI_EXPO_PUSH_URL,
    rotation: {
      enabled: env.RENKI_ACCOUNT_ROTATION === "1" || env.RENKI_ACCOUNT_ROTATION === "true",
      cswapBin: env.RENKI_CSWAP_BIN,
      threshold: env.RENKI_ROTATION_THRESHOLD,
      cooldownMs: env.RENKI_ROTATION_COOLDOWN_MINUTES * 60_000,
      pollMs: env.RENKI_ROTATION_POLL_SECONDS * 1000,
      strategy: env.RENKI_ROTATION_STRATEGY,
      autoRetry: env.RENKI_ROTATION_AUTORETRY !== "0" && env.RENKI_ROTATION_AUTORETRY !== "false",
    },
    rotationConfigPath: env.RENKI_ROTATION_CONFIG ? resolve(env.RENKI_ROTATION_CONFIG) : resolve(dataDir, "rotation-settings.json"),
    usageConfigPath: env.RENKI_USAGE_CONFIG ? resolve(env.RENKI_USAGE_CONFIG) : resolve(dataDir, "usage-accounts.json"),
    usageBaseUrl: env.RENKI_USAGE_BASE_URL,
    usageLoginChannel: env.RENKI_USAGE_LOGIN_CHANNEL,
    usageLoginTimeoutMs: env.RENKI_USAGE_LOGIN_TIMEOUT_SECONDS * 1000,
  };
}
