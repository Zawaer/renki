...
/**
 * Renki's mark: the coil — one and a half turns of a spiral, the same shape as
 * the app icon and favicon (see assets-src/). Work continuing without you,
 * drawn as a line that keeps going round.
 *
 * The viewBox is cropped to the ink rather than the design's 0 0 100 100,
 * because the spiral is asymmetric: its ink sits from x22 to x88, so the raw
 * viewBox would hang it visibly right of centre in the header tile.
 */
export function RenkiMark({ className = "" }: { className?: string }) {
  return (
    <svg viewBox="15.5 5.5 79 89" aria-hidden="true" className={className}>
      <path
        d="M50,32 A18,18 0 0 1 50,68 A28,28 0 0 1 50,12 A38,38 0 0 1 50,88"
        fill="none"
        stroke="currentColor"
        strokeWidth={13}
        strokeLinecap="round"
