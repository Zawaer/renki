...
// @renki/web dev` (a hot-reload dev server, not meant to run 24/7).
//
//   pnpm build                        # compile protocol + daemon + web
//   pm2 start ecosystem.config.cjs    # start both (reads ./.env via the daemon)
//   pm2 save && pm2 startup           # persist across reboots
//   pm2 logs renki-daemon               # tail daemon logs
//   pm2 logs renki-web                  # tail web logs
//
...
// build `pnpm build` produces — rebuild + `pm2 restart renki-web` to deploy a
